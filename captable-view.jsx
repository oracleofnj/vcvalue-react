...
var CapTableEntry = ({seriesName,
  ...
  return (
    <tr>
      <th>{seriesName}</th>
      <td>{shareCount.toLocaleString([],{maximumFractionDigits:0})}</td>
      <td>{prefType === vcvc.LPTYPE_NONE || isNaN(prefPerShare) ? '' : prefPerShare.toLocaleString([],{maximumFractionDigits:4})}</td>
      <td>{prefType}</td>
      <td>{isNaN(invAmt) ?
        'N/A' :
        (invAmt / 1e6 + 'mm @ ' + (preMoney ?
          ((postMoneyValue - invAmt) / 1e6 + 'mm Pre') :
          postMoneyValue / 1e6 + 'mm Post'))
      }</td>
